...
public class Emeny1 : MonoBehaviour
{
    ...
    void Move()
    {
        transform.position = Vector2.MoveTowards(transform.position, player.transform.position, speedEnemy); //в скобках через запятую указаны: текущее положение, куда идти, с какой скоростью
    }
}
